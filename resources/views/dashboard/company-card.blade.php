@foreach($companies as $company)
    ...
    <div class="status-card">
        <div class="card-header">
            <span class="card-header-text">{{ $company->name }}</span>
        </div>
        <ul class="sortable ui-sortable" id="sort{{ $company->id }}" data-company-id="{{ $company->id }}">
            @if($company->employees()->exists())
                @foreach($company->employees as $employee)
                    <li class="text-row ui-sortable-handle" data-employee-id="{{ $employee->id }}">
